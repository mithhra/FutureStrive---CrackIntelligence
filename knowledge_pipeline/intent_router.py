...
Intent types:
  ...
  - "image_crack_analysis" : user uploaded a crack photo — reactive detection pipeline
"""
...
def classify_query(query: str, has_pending_image: bool = False) -> str:
    """
    Classify the user's query into one of 7 intent types.
    ...
    Returns one of:
        ...
        "image_crack_analysis" -> reactive: user uploaded a crack photo

    has_pending_image: pass True from app.py when the user has uploaded an image
    """
    normalized = " ".join(query.lower().strip().split())

    # 0. IMAGE UPLOAD — highest priority, checked before everything else
    #    Triggered when app.py signals that a photo is waiting to be analysed.
    if has_pending_image:
        print(f"[ROUTER] image_crack_analysis <- '{query}' (image pending)")
        return "image_crack_analysis"
