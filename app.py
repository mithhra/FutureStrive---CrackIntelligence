...
if "copilot_history" not in st.session_state:
    st.session_state.copilot_history = [{
        ...
        "content": (
            ...
            "**Try:** *Why is the defect count high?* or *What is the IS 456 curing requirement?* or upload a crack photo below."
        )
    }]
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    with st.expander("📷  Upload Crack Photo — Reactive Detection", expanded=False):
                ...
                if analyse_btn:
                    if not _DETECTOR_AVAILABLE:
                        ...
                        with st.spinner("Running crack detection model..."):
                            ...
                            det["file_name"] = uploaded_file.name
                            det["file_size"] = uploaded_file.size
                            st.session_state.last_image_detection = det

                        # Build user chat message without emojis
        ...
        # Persist detection results (blended overlay + predicted mask + metrics) across reruns
        with col_img_r:
            last_det = st.session_state.last_image_detection
            if last_det:
                show_results = True
                if uploaded_file is not None:
                    if last_det.get("file_name") != uploaded_file.name or last_det.get("file_size") != uploaded_file.size:
                        show_results = False
                else:
                    show_results = False
                ...
                if show_results:
                    ...
                    mask = last_det.get("mask_image")
                    if ann is not None:
                        if mask is not None:
                            # Show overlay + pure mask side by side
                            c_ov, c_msk = st.columns(2)
                            c_ov.image(ann,  caption="Segmentation overlay", use_container_width=True)
                            c_msk.image(mask, caption="Predicted mask",       use_container_width=True)
                        else:
                            st.image(ann, caption="Detection overlay", use_container_width=True)

                    if last_det.get("crack_detected"):
                        sev_colors = {
                            "Minor": "#16A34A", "Moderate": "#D97706",
                            "Severe": "#DC2626", "Critical": "#7C3AED",
                        }
                        sev_c = sev_colors.get(last_det.get("severity_estimate", ""), "#64748B")
                        st.markdown(
                            f"""
                            <div style='background:#F8FAFC;border:1px solid #E2E8F0;
                            border-radius:8px;padding:14px;margin-top:8px;font-size:13px;'>
                            <div style='display:flex;gap:10px;flex-wrap:wrap;'>
                              <div style='flex:1;min-width:90px;text-align:center;'>
                                ...
                                <div style='font-size:18px;font-weight:800;color:#0F172A;'>
                                  {last_det.get('area_fraction', 0):.2f}%
                                </div>
                              </div>
                              <div style='flex:1;min-width:90px;text-align:center;'>
                                <div style='font-size:11px;color:#64748B;font-weight:600;'>SEVERITY</div>
                                <div style='font-size:16px;font-weight:800;color:{sev_c};'>
                                  {last_det.get('severity_estimate', 'N/A')}
                                </div>
                              </div>
                              <div style='flex:1;min-width:90px;text-align:center;'>
                                <div style='font-size:11px;color:#64748B;font-weight:600;'>INSTANCES</div>
                                <div style='font-size:18px;font-weight:800;color:#0F172A;'>
                                  {last_det.get('num_instances', 0)}
                                </div>
                              </div>
                              <div style='flex:1;min-width:90px;text-align:center;'>
                                <div style='font-size:11px;color:#64748B;font-weight:600;'>EST. WIDTH</div>
                                <div style='font-size:18px;font-weight:800;color:#0F172A;'>
                                  {last_det.get('estimated_width_mm', 0):.2f} mm
                                </div>
                              </div>
                              <div style='flex:1;min-width:90px;text-align:center;'>
                                <div style='font-size:11px;color:#64748B;font-weight:600;'>CONFIDENCE</div>
                                <div style='font-size:18px;font-weight:800;color:#0F172A;'>
                                  {last_det.get('confidence', 0)*100:.1f}%
                                </div>
                              </div>
                            </div>
                            </div>
                        ...
                        st.markdown(
                            ...
                            "No crack detected in this image."
                            "</div>",
                            unsafe_allow_html=True,
                        )
